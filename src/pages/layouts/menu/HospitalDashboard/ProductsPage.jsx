...
//   const [formErrors, setFormErrors] = useState({});// add | edit | confirmDelete | viewProfile
...
//       { label: "OT EQUIPMENT", value: "OT EQUIPMENT" },
//       { label: "ICU", value: "ICU" },
...
//   { name: "gst", label: "GST/VAT %" },
//   { name: "salesPrice", label: "Sales Price" },
//   { name: "discPercent", label: "Disc %" },
//   { name: "finalSalesPrice", label: "Final Sales Price" },
//   { name: "buyPrice", label: "Buy Price" },
//   {
//     name: "itemType",
//     label: "Item Type",
//     type: "select",
...
//   { name: "numberStripBox", label: "Number Strip Box" },
//   { name: "stripQuantity", label: "Strip Quantity" },
//   { name: "minSalePrice", label: "Min Sale Price" },
//   { name: "sizeDesc", label: "Size-Desc/Degree" },
//   { name: "avgBuyPrice", label: "Avg Buy Price" },
//   { name: "expireDate", label: "Expire Date", type: "date" },
//   { name: "doseType", label: "Dose/Sample Type" },
...
// ]);
// useEffect(() => {
//   if (!isOpen && reopenProductForm) {
//     setIsOpen(true);
//     setReopenProductForm(false);
//   }
// }, [isOpen, reopenProductForm]);
//   const filterProduct = [
//     {
//       key: "combinedFilter",
//       options: productFields
//         .filter(f => f.type === "select")
//         .flatMap(f => f.options.map(opt => ({
//           value: opt.value,
//           label: opt.label
//         })))
//     }
//   ];
//   const viewFields = productFields.map((f, idx) => ({
//   key: f.name,
//   label: f.label,
//   titleKey: idx === 0, // productType
//   initialsKey: idx === 0,
//   subtitleKey: idx === 1 // category
// }));
...
//     if (!data.itemId) errors.itemId = "Item ID is required.";
//     if (!data.itemName) errors.itemName = "Item Name is required.";
//     if (!data.salesPrice) errors.salesPrice = "Sales Price is required.";
//     if (!data.status) errors.status = "Status is required.";

//     // Example: price fields must be numbers >= 0
//     const numberFields = ["basicPrice", "salesPrice", "discPercent", "finalSalesPrice", "buyPrice", "avgBuyPrice"];
...
//   if (addingField) {
//     const value = data[addingField]?.trim();
//     if (!value) {
...
//   } else if (mode === "edit" && currentRow) {
//     setRows(prev => prev.map(row => (row.id === currentRow.id ? { ...row, ...data } : row)));
//   }
...
//   const handleDelete = () => {
//     if (currentRow) {
//       setRows(prev => prev.filter(row => row.id !== currentRow.id));
//       setCurrentRow(null);
//     }
//   };
...
//         <h1 className="h4-heading">Product Lists</h1>
//         <button
//           onClick={() => {
//             setMode("add");
//             setCurrentRow(null);
...
//           { header: "Basic Price", accessor: "basicPrice" },
//          { header: "Status",accessor: "status",
//        cell: (row) => {const key = row.status ? row.status.toLowerCase() : "";return (
//         <span className={`px-2 py-1 rounded-full text-xs font-semibold ${ statusColors[key] || "text-gray-600 bg-gray-100" }`}> {key.toUpperCase()} </span>
//       );
//     },
//   },
//           {
...
//         onCellClick={(row, col) => {
//           if (col.accessor === "productType") handleView(row);
//         }}
//       />
...
//   title={
//     addingField
//       ? `Add New ${addingField === "category" ? "Category" : "Item Type"}`
//       : mode === "add"
//         ? "Add New Product"
//         : mode === "edit"
//           ? "Edit Product"
...
//   fields={
//     addingField
//       ? [{ name: addingField, label: `New ${addingField} Name` }]
//       : productFields
//   }
