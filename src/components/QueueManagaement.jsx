...
const QueueManagement = ({ tokens, onTokensUpdate }) => {
  ...
  useEffect(() => {
    ...
    const stats = {
      ...
      urgent: todayTokens.filter(t => t.priority === 'urgent').length,
                ...
                {filteredTokens.length === 0 ? (
                  ...
                  filteredTokens.map((token) => (
                    <tr key={token.id} className="border-t hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${token.priority === 'emergency' ? 'bg-red-100 text-red-800' : token.priority === 'urgent' ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}`}>{token.priority}</span></td>
