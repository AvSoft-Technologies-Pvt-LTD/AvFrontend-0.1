...
const TokenGenerator = ({ onTokenGenerated }) => {
  ...
  const handleSubmit = async e => {
    ...
    const newToken = {
      ...
      estimatedTime: (() => { const base = new Date(); base.setMinutes(base.getMinutes() + (formData.priority === 'emergency' ? 5 : formData.priority === 'urgent' ? 15 : 30)); return base.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); })()
    };
                      ...
                      <label key={priority.value} className="cursor-pointer">
                        ...
                        <div className={`p-3 rounded-lg border-2 transition-all ${formData.priority === priority.value ? `${priority.bg} border-current ${priority.color}` : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                          <div className="text-center">
                            ...
                            <div className="text-xs text-gray-500 mt-1">~{priority.value === 'emergency' ? '5' : priority.value === 'urgent' ? '15' : '30'} min</div>
                          </div>
                        </div>
                      </label>
                    ))}
